...
case $1 in
    "pipeline_reduc" | "pipeline_general_reduc" | "reduc_memdep" | "dual_pipeline_memdep" | "waw" | "waw_reduc" | "waw_reduc_noSB" | "waw_noSB" | "waw_reduc_code_spec")
        ...
        if [ "$3" == "before" ]; then
        	a="$a src/tricore-SB-stalling-before.ucl"
        else
        	a="$a src/tricore-SB.ucl"
        fi
        a="$a test/param-$1.ucl test/test.ucl test/conditions-notiming-anomaly.ucl test/property-notiming-anomaly.ucl -no-version-check"
        echo $a
        eval $a
    ;;
